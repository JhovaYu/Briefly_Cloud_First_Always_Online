"""
SQLAlchemy async engine and session factory helpers for schedule-service.

The async engine is created only when SCHEDULE_STORE_TYPE=postgres.
When SCHEDULE_STORE_TYPE=inmemory, this module is not used.
"""
...
def get_database_url(settings: Settings) -> str:
    ...
    if settings.SCHEDULE_DATABASE_URL:
        return settings.SCHEDULE_DATABASE_URL
    raise ValueError(
        "SCHEDULE_DATABASE_URL is required when SCHEDULE_STORE_TYPE=postgres. "
...
def create_async_engine_if_not_exists(settings: Settings):
    ...
    logger.info("Schedule async engine created (lazy connection)")
    return _engine
